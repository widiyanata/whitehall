module Edition::AuditTrail
  ...
  def record_create
    versions.create event: 'create', whodunnit: Edition::AuditTrail.whodunnit.id, state: state
  end
  private :record_create

  def record_update
    if changed.any?
      versions.build event: 'update', whodunnit: Edition::AuditTrail.whodunnit.id, state: state
    end
  end
